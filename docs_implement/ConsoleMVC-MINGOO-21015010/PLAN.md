...
콘솔 애플리케이션에서 **Model / View / Controller 역할을 명확히 분리**한 최소 골격(Skeleton)을 만들어보고
직접 구현·코드 리딩을 통해 구조적 패턴을 학습하는 참고용 PoC다.
비즈니스 로직(반도체 시료 도메인)은 최소화하고, "메뉴 입력 → 처리 → 화면 출력" 흐름 자체의 구조에 집중한다.

> PoC와 Main의 관계(코드 재사용 여부, 리뷰/TDD 기대 수준)는
> [Overall/PLAN.md](../Overall/PLAN.md) 3절 참고. 세부 클래스 설계는 자유롭게 실험해도 된다.
...
### Phase 5 — 통합 및 (가벼운) 테스트
- Controller에 대해 실패하는 테스트를 먼저 작성 → 최소 구현으로 통과시키는 간단한 TDD 사이클을
  1~2개 핵심 시나리오(예: 정상 입력 시 라우팅, 잘못된 입력 시 처리)에 대해 수행
- 가짜(Mock) View를 주입하여 입력값별 분기 검증
